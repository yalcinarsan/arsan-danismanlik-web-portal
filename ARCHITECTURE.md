...
- Makale kapağı isteğe bağlı. Kullanıldığında frontmatter içindeki masaüstü ve mobil görsel yolları, 1200×630 paylaşım görseli, `alt` ve `altyazi` alanları birlikte tanımlanıyor. Kapak, metin ile içerik navigasyonunun ortak genişliğini kullanıyor; metin ve sağdaki navigasyon kapağın altında başlıyor. Tarayıcı uygun boyuttaki WebP’yi seçiyor; paylaşım görseli Open Graph, X kartı ve Article yapılandırılmış verisinde kullanılıyor.
...
**2026-08-19 — Kurum görünümü: erişim ve maskeleme sunucuya taşındı (Faz 1.5)**
Kurum demo ekranı `adaylar` tablosunu `select('*')` ile çekip maskelemeyi tarayıcıda yapıyordu; erişim de `KurumDemo.tsx` içindeki sabit e-posta karşılaştırmasıydı. Yalnızca Yalçın'ın ekran paylaşımıyla gösterdiği sürece sorun değildi, ama kurum temsilcilerinin **kendi başlarına girmesi** istendiği anda ikisi de yetersiz: tarayıcıya inen ham satır DevTools ile görülür, JavaScript karşılaştırması atlanır. Çözüm `public_stats()` ile aynı desen — `security definer` fonksiyon RLS'i baypas eder ama yalnızca güvenli alanları döndürür; farkı agregat değil satır döndürmesi, o yüzden bir de davetli listesi var. Eklenenler: `kurum_erisim` tablosu (RLS açık, **bilerek politikasız** → yalnızca service_role ve fonksiyon görür) ve `kurum_havuzu()` fonksiyonu. **Hassas sütunlar fonksiyonun dönüş tipinde hiç yok** (ad, e-posta, telefon, son_kurum, son_pozisyon, serbest_metin, cv_path) — arayüz hatası bile sızdıramaz. Şehir yerine **bölge** dönüyor: 34 kişilik havuzda şehir + kanal + kıdem + marka birleşimi kimliği tahmin ettirebiliyor, dışarıya açarken bir kademe geri çekildi (Yalçın kendi gösterirken şehir görüyordu). Faz 2 beklenmedi; bu, kurumsal tarafın kendisi değil yalnızca görüntüleme erişimi. Giriş bağlantısı da listeye bağlandı: `kurum_erisimi_var()` (yalnızca boolean döner, liste sızmaz) form gönderilmeden önce sorgulanıyor, tanımlı olmayan adrese hiç e-posta çıkmıyor ve Supabase'de boşuna hesap açılmıyor. Bu **akış kontrolü**, güvenlik sınırı değil — biri bu adımı atlayıp kendine link yollatsa bile girişte `kurum_havuzu()` 403 döner. Dosya: `supabase/kurum-erisim.sql`. **Not:** `AdaylarListesi.tsx` (maskesiz yönetici listesi) hâlâ eski desende — sabit e-posta + doğrudan tablo sorgusu; oradaki gerçek sınır RLS ve `adaylar` üzerindeki canlı SELECT politikasının `schema.sql` ile aynı olduğu **teyit edilmeli.**
